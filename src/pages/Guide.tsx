...
const GUIDE_STEPS = [
  ...
  {
    icon: <Map className="w-8 h-8 text-[#FF007A]" />,
  ...
  {
    icon: <ShieldAlert className="w-8 h-8 text-[#CCFF00]" />,
  ...
  {
    icon: <Sparkles className="w-8 h-8 text-[#FF007A]" />,
  ...
  {
    icon: <Trophy className="w-8 h-8 text-[#CCFF00]" />,
...
export default function Guide() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-6 relative z-10 text-white overflow-y-auto print:bg-black">
      <style>
        {`
          @media print {
            @page {
      ...
      <div className="w-full max-w-2xl">
        <h1 className="text-4xl md:text-5xl font-black uppercase italic mb-2 text-center text-white">
          Manual del <span className="text-[#FF007A]">Runner</span>
        </h1>
        ...
        <div className="space-y-6 relative before:absolute before:inset-0 before:ml-8 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-[#00F0FF] before:via-[#FF007A] before:to-[#CCFF00]">
          {GUIDE_STEPS.map((step, idx) => (
            <motion.div 
              ...
              className={`relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active break-inside-avoid mb-6`}
            >
              {/* Icon Marker */}
              <div className={`flex items-center justify-center w-16 h-16 rounded-full border-4 border-black shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 bg-black/80 backdrop-blur-md z-10 ${
              ...
              {/* Card */}
              <div className="w-[calc(100%-5rem)] md:w-[calc(50%-3rem)] bg-white/5 backdrop-blur-xl border border-white/10 p-6 rounded-3xl shadow-xl hover:bg-white/10 transition-colors">
                <h3 className="text-xl font-black italic uppercase mb-2 text-white">{step.title}</h3>
